    ...
    if (json.matchData == undefined) {
      ...
      for (let i = 0; i < json.matchData.players.length; i++) {
        if (json.matchData.players[i].matchPeriodData.length !== 0) {
          const steamid = convertSteamID(
            json.matchData.players[i].info.steamId
          );
          ...
          for (
            ...
            expectedgoals =
              expectedgoals +
              json.matchData.players[i].matchPeriodData[j].statistics[28];
            if (
              !positions.some(
                e =>
                  e.position ===
      ...
      for (let i = 0; i < matchevents.length; i++) {
        for (let j = 0; j < json.matchData.players.length; j++) {
          if (
            convertSteamID(matchevents[i].player1SteamId) ===
            convertSteamID(json.matchData.players[j].info.steamId)
          ) {
            matchevents[i].name = removeTag(
              json.matchData.players[j].info.name
            );
          } else if (
            convertSteamID(matchevents[i].player2SteamId) ===
            convertSteamID(json.matchData.players[j].info.steamId)
          ) {
            ...
            convertSteamID(matchevents[i].player3SteamId) ===
            convertSteamID(json.matchData.players[j].info.steamId)
          ) {
      ...
      for (let i = 0; i < matchevents.length; i++) {
        if (matchevents[i].player1SteamId) {
          matchevents[i].player1SteamId = convertSteamID(
            matchevents[i].player1SteamId
          );
        }
        if (matchevents[i].player2SteamId) {
          matchevents[i].player2SteamId = convertSteamID(
            matchevents[i].player2SteamId
          );
        }
        if (matchevents[i].player3SteamId) {
          matchevents[i].player3SteamId = convertSteamID(
        ...
        teams: [
          {
            ...
            statistics: {
              ...
              expectedgoals: json.matchData.teams[0].matchTotal.statistics[28]
            },
          ...
          },
          {
            ...
            statistics: {
              ...
              expectedgoals: json.matchData.teams[1].matchTotal.statistics[28]
            },
