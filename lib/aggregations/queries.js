function getRegex(arg) {
  ...
  if (tempregex.test(arg)) {
    ...
    return { $regex: "(Liga D1|Promoción D1)" };
  } else if (arg === "d2") {
    return { $regex: "(Liga D2|Promoción D2)" };
  } else if (arg === "d3") {
    return { $regex: "(Liga D3|Promoción D3)" };
  } else if (arg === "master") {
    ...
    return { $regex: "(Liga D1|Promoción D1).*? T" + match[1] };
  } else if (d2regex.test(arg)) {
    const match = arg.match(d2regex);
    return { $regex: "(Liga D2|Promoción D2).*? T" + match[1] };
  } else if (d3regex.test(arg)) {
    const match = arg.match(d3regex);
    return { $regex: "(Liga D3|Promoción D3).*? T" + match[1] };
