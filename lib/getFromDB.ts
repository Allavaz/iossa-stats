...
export async function logoFilenameExists(
  logofilename: string
): Promise<boolean> {
  const client = await clientPromise;
  const db = client.db();
  const doc = await db
    .collection("teams")
    .findOne({ logofilename }, { projection: { _id: 1 } });
  return doc !== null;
}
...
export async function updateTeam(
  ...
  await db
    .collection("teams")
    .updateOne({ name }, { $set: { shortname, logofilename } });
}

export async function updateTeamAliases(
  name: string,
  aliases: string[]
): Promise<void> {
  const client = await clientPromise;
...
export async function getTeamMatchCounts(): Promise<Record<string, number>> {
  ...
  return Object.fromEntries(
    docs.map(d => [d._id as string, d.count as number])
  );
}
