  ...
  for (let i = 0; i < data.length; i++) {
    for (let j = 0; j < data[i].players.length; j++) {
      if (data[i].players[j].info.steam_id === id) {
        ...
        if (
          lastpos === undefined &&
          data[i].players[j].statistics.positions.length > 0
        ) {
    ...
    for (let j = 0; j < data[i].players.length; j++) {
      if (data[i].players[j].info.steam_id === id) {
        ...
        shotsontarget =
          shotsontarget + data[i].players[j].statistics.shotsontarget;
        passes = passes + data[i].players[j].statistics.passes;
        passescompleted =
          passescompleted + data[i].players[j].statistics.passescompleted;
        interceptions =
          interceptions + data[i].players[j].statistics.interceptions;
        ...
        distancecovered =
          (distancecovered + data[i].players[j].statistics.shots) / 2;
        possession =
          (possession + data[i].players[j].statistics.possession) / 2;
        ...
        tacklescompleted =
          tacklescompleted + data[i].players[j].statistics.tacklescompleted;
        foulssuffered =
          foulssuffered + data[i].players[j].statistics.foulssuffered;
        savescaught = savescaught + data[i].players[j].statistics.savescaught;
        goalkicks = goalkicks + data[i].players[j].statistics.goalkicks;
        goalsconceded =
          goalsconceded + data[i].players[j].statistics.goalsconceded;
        secondsplayed =
          secondsplayed + data[i].players[j].statistics.secondsplayed;
        for (
          let k = 0;
          k < data[i].players[j].statistics.positions.length;
          k++
        ) {
          if (
            !positions.some(
              e =>
                e.position ===
                data[i].players[j].statistics.positions[k].position
            )
          ) {
            positions.push({
            ...
            positions[index].seconds =
              positions[index].seconds +
              data[i].players[j].statistics.positions[k].seconds;
          }
        }
