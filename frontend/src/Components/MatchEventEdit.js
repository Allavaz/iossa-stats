...
export default function MatchEventEdit(props) {
  ...
  if (props.side === 'home') {
    if ((props.item.team === 'home' && props.item.event !== 'OWN GOAL') || (props.item.team === 'away' && props.item.event === 'OWN GOAL')) {
      return (
        <li key={props.index}>
          <div className='matchevent' onMouseOver={e => setEventHovering(true)} onMouseOut={e => setEventHovering(false)}>
            ...
            <FontAwesomeIcon style={{marginLeft: '5px', cursor: 'pointer', opacity: eventHovering ? '100%' : '0%' }} icon={faEdit} onClick={e => props.setEventEditing(props.index)}></FontAwesomeIcon>
            <FontAwesomeIcon style={{marginLeft: '5px', cursor: 'pointer', opacity: eventHovering ? '100%' : '0%' }} icon={faTrashAlt} onClick={e => props.onRemoveEvent(props.index)}></FontAwesomeIcon>
          </div>
        </li>
      );
    } else {
